...
namespace dd{
  ...
  public:
    ...
    // TCLAP::ValueArg<std::string> * inc_normal_run_folder;
    ...
    CmdParser(std::string _app_name){
      ...
      if(app_name == "gibbs" || app_name == "cnn"){
        ...
        // inc_normal_run_folder = new TCLAP::ValueArg<std::string>("n","normal","(Incremental) normal run folder",false,"","string");

        n_learning_epoch = new TCLAP::ValueArg<int>("l","n_learning_epoch","Number of Learning Epochs",true,-1,"int");
        n_samples_per_learning_epoch = new TCLAP::ValueArg<int>("s","n_samples_per_learning_epoch","Number of Samples per Leraning Epoch",false,-1,"int");

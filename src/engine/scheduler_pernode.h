...
public:
    ...
    double exec(const long *const tasks, int ntasks,
        ...
        std::cout << "| Running on " << n_sharding << " Nodes with " << n_thread_per_numa << " Cores Each..."
                  << std::endl;
        ...
        for (int i_sharding = 0; i_sharding < n_sharding; i_sharding++) {
            for (int i_thread = 0; i_thread < n_thread_per_numa; i_thread++) {
                ...
                if (DATAREPL == DW_DATAREPL_FULL) {
                    futures.push_back(std::async(std::launch::async, _pernode_run_map<RDTYPE, WRTYPE>, p_map, RDPTR,
                                                 model_replicas[i_sharding], tasks, 0, ntasks, i_sharding));
                } else {
                    futures.push_back(std::async(std::launch::async, _pernode_run_map<RDTYPE, WRTYPE>, p_map, RDPTR,
                                                 model_replicas[i_sharding], tasks, start, end, i_sharding));
                }
            ...
            if (isjulia == true) {
                ...
                comm_threads.push_back(
                        std::thread(_pernode_comm<RDTYPE, WRTYPE>, p_comm, model_replicas[i_sharding], model_replicas,
                                    n_sharding, i_sharding));

    ...
    double exec(const long *const tasks, int ntasks,
        ...
        for (int i_sharding = 0; i_sharding < n_sharding; i_sharding++) {
            ...
            if (DATAREPL == DW_DATAREPL_FULL) {
                futures.push_back(std::async(std::launch::async, _percore_run_map<RDTYPE, WRTYPE>, p_map, RDPTR,
                                             model_replicas[i_sharding], tasks, 0, ntasks));
            } else {
                futures.push_back(std::async(std::launch::async, _percore_run_map<RDTYPE, WRTYPE>, p_map, RDPTR,
                                             model_replicas[i_sharding], tasks, start, end));
            }
        }

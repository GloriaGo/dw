...
public:
    ...
    SparseDimmWitted(
            ...
            current_handle_id(0),
            row_pointers((SparseVector < A > *)

    ::operator new(_n_rows
    * sizeof(SparseVector <A>))),
    col_pointers((SparseVector<A>
    *)
    ::operator new(_n_cols
    * sizeof(SparseVector <A>))),
    c2r_col_2_rowbuffer_idxs(new Pair<long, long>[_n_cols]),
    row_ids(new long[_n_rows]),
    col_ids(new long[_n_cols]),

    dw_row_runner (DWRun<TASK_ROW_SPARSE<A, B>, B, model_repl_type, data_repl_type>( &

    task_row, p_model, sparse_model_allocator<A, B>)),

    dw_col_runner (DWRun<TASK_COL_SPARSE<A, B>, B, model_repl_type, data_repl_type>( &

    task_col, p_model, sparse_model_allocator<A, B>)),

    dw_c2r_runner (DWRun<TASK_C2R_SPARSE<A, B>, B, model_repl_type, data_repl_type>( &

    task_c2r, p_model, sparse_model_allocator<A, B>))
    {
        ...
        if (access_mode == DW_ACCESS_COL || access_mode == DW_ACCESS_C2R) {
            ...
            for (int i = 0; i < n_cols; i++) {
                if (i == n_cols - 1) {
                    col_pointers[i] = SparseVector < A >
                                      (&_new_ele_buffer[colpointers_ori[i]], &colrows[colpointers_ori[i]], n_elems -
                                                                                                           colpointers_ori[i]);
                } else {
                    col_pointers[i] = SparseVector < A >
                                      (&_new_ele_buffer[colpointers_ori[i]], &colrows[colpointers_ori[i]],
                                              colpointers_ori[i + 1] - colpointers_ori[i]);
                }
            }

...
public:
    ...
    DenseDimmWitted(A **_data, long _n_rows, long _n_cols, B *const _model
            ...
            current_handle_id(0),
            row_pointers((DenseVector < A > *)

    ::operator new(_n_rows
    * sizeof(DenseVector <A>))),
    col_pointers((DenseVector<A>
    *)
    ...
    * sizeof(DenseVector <A>))),
    ...
    dw_row_runner (DWRun<TASK_ROW<A, B>, B, model_repl_type, data_repl_type>( &

    task_row, p_model, dense_model_allocator<A, B>)),

    dw_col_runner (DWRun<TASK_COL<A, B>, B, model_repl_type, data_repl_type>( &

    task_col, p_model, dense_model_allocator<A, B>)),

    dw_c2r_runner (DWRun<TASK_C2R<A, B>, B, model_repl_type, data_repl_type>( &

    task_c2r, p_model, dense_model_allocator<A, B>))
    {
        ...
        if (access_mode == DW_ACCESS_ROW) {
            ...
            if (access_mode == DW_ACCESS_C2R) {
                ...
                _c2r_row_pointers_buffer = (DenseVector < A > *)
                ::operator new(n_rows * n_cols * sizeof(DenseVector < A > ));
                long ct = 0;
                for (int j = 0; j < n_cols; j++) {
